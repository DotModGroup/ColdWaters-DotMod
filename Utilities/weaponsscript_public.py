file = open(input('Input the complete file directory of where your weapons.txt is located\n> '), encoding='utf8')
save_location = input('Input the complete file directory of where your output should go.'
...
for line in file:
    if line == '\n':
    ...
    elif line.startswith('WeaponObjectReference'):
        weapon_name = line[22:]
        weapon = line
        weapon_type = 'weapon_'
        ...
        weapon += line
        file_save = open('%s\\%s%s.txt'
                         % (save_location, weapon_type, weapon_name[0:-1]), encoding='utf8', mode='a')
